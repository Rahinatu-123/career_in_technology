        ...
        switch ($_POST['action']) {
            case 'add':
                ...
                if (!empty($word) && !empty($definition)) {
                    ...
                    try {
                        // Insert tech word
                        $stmt = $conn->prepare("INSERT INTO tech_words (word, definition, example, category) VALUES (?, ?, ?, ?)");
                        $stmt->bind_param("ssss", $word, $definition, $example, $category);
                        $stmt->execute();
                        $word_id = $conn->insert_id;
                        
                        // Insert career relationships
                        if (!empty($career_ids)) {
                            $stmt = $conn->prepare("INSERT INTO word_careers (word_id, career_id) VALUES (?, ?)");
                            foreach ($career_ids as $career_id) {
                                $stmt->bind_param("ii", $word_id, $career_id);
                                $stmt->execute();
                            }
                        }
            ...
            case 'edit':
                ...
                if ($id > 0 && !empty($word) && !empty($definition)) {
                    ...
                    try {
                        // Update tech word
                        $stmt = $conn->prepare("UPDATE tech_words SET word = ?, definition = ?, example = ?, category = ? WHERE id = ?");
                        $stmt->bind_param("ssssi", $word, $definition, $example, $category, $id);
                        $stmt->execute();
                        
                        // Delete existing career relationships
                        $stmt = $conn->prepare("DELETE FROM word_careers WHERE word_id = ?");
...
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    ...
    if ($result->num_rows === 1) {
        ...
        $stmt->execute();
        $result = $stmt->get_result();
        $edit_word['career_ids'] = [];
        while ($row = $result->fetch_assoc()) {
            $edit_word['career_ids'][] = $row['career_id'];
        }
    }
}
?>
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            <ul>
                ...
                <li><a href="tech_words.php" class="active"><i class="fas fa-language"></i> Tech Words</a></li>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addTechWordModal">
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <?php if (empty($tech_words)): ?>
                                        <tr>
                                            <td colspan="7" class="text-center">No tech words found.</td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($tech_words as $word): ?>
                                            <tr>
                                                ...
                                                <td><?php echo htmlspecialchars($word['career_titles'] ?? 'None'); ?></td>
    ...
    <!-- Add Word Modal -->
    <div class="modal fade" id="addTechWordModal" tabindex="-1" aria-labelledby="addTechWordModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addTechWordModalLabel">Add New Tech Word</h5>
    ...
    <!-- Edit Word Modals -->
    <?php foreach ($tech_words as $word): ?>
    <div class="modal fade" id="editWordModal<?php echo $word['id']; ?>" tabindex="-1" aria-labelledby="editWordModalLabel<?php echo $word['id']; ?>" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editWordModalLabel<?php echo $word['id']; ?>">Edit Tech Word</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="POST" action="">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="word<?php echo $word['id']; ?>" class="form-label">Word</label>
                            <input type="text" class="form-control" id="word<?php echo $word['id']; ?>" name="word" value="<?php echo htmlspecialchars($word['word']); ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="definition<?php echo $word['id']; ?>" class="form-label">Definition</label>
                            <textarea class="form-control" id="definition<?php echo $word['id']; ?>" name="definition" rows="3" required><?php echo htmlspecialchars($word['definition']); ?></textarea>
                        </div>
                        <div class="mb-3">
                            <label for="example<?php echo $word['id']; ?>" class="form-label">Example</label>
                            <textarea class="form-control" id="example<?php echo $word['id']; ?>" name="example" rows="3"><?php echo htmlspecialchars($word['example']); ?></textarea>
                        </div>
                        <div class="mb-3">
                            <label for="category<?php echo $word['id']; ?>" class="form-label">Category</label>
                            <input type="text" class="form-control" id="category<?php echo $word['id']; ?>" name="category" value="<?php echo htmlspecialchars($word['category']); ?>">
                        </div>
                        <div class="mb-3">
                            <label for="career_ids<?php echo $word['id']; ?>" class="form-label">Related Careers</label>
                            <select class="form-select" id="career_ids<?php echo $word['id']; ?>" name="career_ids[]" multiple>
                                <?php 
                                $word_careers = explode(',', $word['career_titles'] ?? '');
                                foreach ($careers as $career): 
                                    $selected = in_array($career['title'], $word_careers) ? 'selected' : '';
                                ?>
